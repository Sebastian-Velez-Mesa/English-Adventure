...
if (html.includes('<body>') && !html.includes('id="zen-mascot"')) {
    html = html.replace('<body>', '<body>\n' + svg_defs);
}

// Global replacements of old emoji
html = html.replace(/<span class="zen-emoji">🦊<\/span>/g, '<svg class="zen-icon zen-icon-nav"><use href="#zen-mascot"></use></svg>');
html = html.replace(/<div class="intro-zen-big">🦊<\/div>/g, '<svg class="zen-icon intro-zen-big"><use href="#zen-mascot"></use></svg>');
html = html.replace(/<div class="intro-zen-big spin-in">🦊<\/div>/g, '<svg class="zen-icon intro-zen-big spin-in"><use href="#zen-mascot"></use></svg>');
html = html.replace(/🦊 Zen/g, '<svg class="zen-icon"><use href="#zen-mascot"></use></svg> Zen');
html = html.replace(/🦊 <strong>Zen says:<\/strong>/g, '<svg class="zen-icon"><use href="#zen-mascot"></use></svg> <strong>Zen says:<\/strong>');
html = html.replace(/🦊/g, '<svg class="zen-icon"><use href="#zen-mascot"></use></svg>'); // Catch all other emojis

let main_zen_raw = svg_defs.replace('<svg width="0" height="0" style="position:absolute; width:0; height:0; display:none;">', '')
  .replace('</svg>', '')
  .replace('<symbol id="zen-mascot" viewBox="0 0 120 148" overflow="visible">', '<svg id="zen-main-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 148" style="overflow:visible;">')
  .replace('</symbol>', '</svg>').trim();

let html_parts = html.split('<!-- ===== ZEN FLOATING GUIDE ===== -->');
if (html_parts.length > 1) {
    let end_parts = html_parts[1].split('<!-- ===== FEEDBACK MODAL ===== -->');
...
console.log('index.html updated successfully via Node!');
